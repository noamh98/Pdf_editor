...
- **Nobody has launched it on Windows.** CI builds, tests and packages on Windows and that job passes, so the suite is green on the target platform — but no person has double-clicked the executable, drawn a window or opened a file picker there.
